 ...
 */
// getExtraTimeNoteHtml removed — ET/PEN info is already in the FT badge (getCompletedBadgeHtml)
...
/**
 * Render 3rd Place and Final matches in the hero section using calendar__match format.
 * Uses the same card style as the calendar, with the Final slightly more prominent.
 */
function renderTodayMatches() {
  ...
  const matches = [];
  if (tpMatch) matches.push({ ...tpMatch, _isFinal: false });
  if (finalMatch) matches.push({ ...finalMatch, _isFinal: true });
  ...
  matches.forEach(match => {
    ...
    if (match._isFinal) matchClasses.push('calendar__match--final');
    ...
    const stageNames = { tp: '3er Puesto', final: 'Final' };
    const groupLabel = match.stage ? (stageNames[match.stage] || match.stage) : '';
...
function renderUpcomingMatches() {
  ...
  upcoming.forEach(match => {
    ...
    else if (isFullTime) statusBadge = `<div class="upcoming__match-badge"><span class="ft-badge">90\'</span></div>`;
...
function initBracket() {
  ...
  // Each feeder is 1 row → offset = 121/2 ≈ 61px
  for (let i = 0; i < 8; i++) {
    const rs = i * 2 + 1;
    html += buildConnector(2, rs, rs + 2, 61);
  }
  ...
  // Each feeder is 2 rows → offset = (2×121 + 6) / 2 = 124px
  for (let i = 0; i < 4; i++) {
    const rs = i * 4 + 1;
    html += buildConnector(4, rs, rs + 4, 124);
  }
  ...
  // Each feeder is 4 rows → offset = (4×121 + 3×6) / 2 = 251px
  for (let i = 0; i < 2; i++) {
    const rs = i * 8 + 1;
    html += buildConnector(6, rs, rs + 8, 251);
  }
  ...
  // Each feeder is 8 rows → offset = (8×121 + 7×6) / 2 = 505px
  html += buildConnector(8, 1, 17, 505);
...
function renderBracketMatch(match, isFinal = false) {
  ...
  else if (isFullTime) statusHtml = '<span class="ft-badge">90\'</span>';
